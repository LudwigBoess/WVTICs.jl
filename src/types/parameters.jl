...
#
#   - `Parameters`         : parameter-file values
#   - `ProblemParameters`  : per-problem geometry
#   - `KernelConfig`       : runtime DESNNGB/NNGBDEV/NGBMAX plus the kernel
#                            instance, looked up from the tables below

"""
    Parameters

Parameter-file values, populated by [`read_param_file`](@ref) from the ASCII
parameter file.
...
- `density_function_correction` : artificial density-model correction factor
                              ...
                              (parameter-file tag `BiasCorrection` is also
                              accepted)
...
- `DesNumNgb`               : target SPH neighbour count (`DESNNGB`). `0` means
...
Per-problem geometry, filled by the problem setup.
...
- `Boxsize`  : box dimensions; **`Boxsize[1]` is ALWAYS the largest axis**
               (neighbour finding relies on it)
...
Enumeration of the selectable SPH kernels. `WendlandC10`/`WendlandC12` are
listed for completeness but are **not** available in `SPHKernels.jl` and are
unsupported.
"""
...
# 2D values: cubic 14/0.01, WC2 16/0.01, all other kernels 44/0.01.
...
Runtime `DESNNGB`/`NNGBDEV`/`NGBMAX` configuration plus the kernel instance
...
(looks up `DESNNGB`/`NNGBDEV` from the tables) or from any kernel instance via
[`KernelConfig(kernel::AbstractSPHKernel; desnngb, …)`](@ref).
"""
...
`DESNNGB`/`NNGBDEV` from the kernel+dimension tables. `NGBMAX` is
`DESNNGB * 16` for the cubic spline and `DESNNGB * 8` otherwise. The matching
`SPHKernels.jl` instance is built once and stored.
...
function KernelConfig(kernel::KernelType; dim::Integer = 3)
    ...
    if kernel === WendlandC10 || kernel === WendlandC12
        throw(ArgumentError("kernel $kernel is not available in SPHKernels.jl " *
                            "and is unsupported in this port."))
    end
...
`desnngb` (the target SPH neighbour count, `DESNNGB`) is **required**: there is
no built-in `DESNNGB` table for arbitrary kernels. `nngbdev` (the
...
The default kernel: Wendland C4, DESNNGB 200.
"""
